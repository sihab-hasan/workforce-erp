...
export function SocialLoginButtons({
  ...
  async function handleSocial(provider: SsoProvider) {
    ...
    try {
      ...
      sessionStorage.setItem(`workforce-erp.sso.${provider}.state`, response.state);
      sessionStorage.setItem(`workforce-erp.sso.${provider}.returnTo`, returnTo);
      window.location.assign(response.redirect_url);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "We couldn’t start single sign-on. Please try again.",
      );
