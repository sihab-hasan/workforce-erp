...
export function ClockActionWidget({ employeeId, className }: ClockActionWidgetProps) {
  ...
  const handleClockIn = () => {
    const payload = employeeId === undefined ? {} : { employee_id: employeeId };
  ...
  const handleClockOut = () => {
    ...
    clockOutMutation.mutate(payload, {
      ...
      onError: (err: Error) => {
        const message = err.message || "Failed to clock out. Please try again.";
    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {/* Clock In Timestamp */}
          <div className="rounded-xl border border-border/60 bg-muted/30 p-3.5">
            <p className="text-xs font-medium text-muted-foreground">Clock In Time</p>
            {isTodayPending ? (
        ...
        {!isTodayPending && !isTodayError && !hasEmployeeProfile && (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
            ...
            <span>
              Time clock is unavailable until this account is linked to an employee profile.
            </span>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          ...
          <Button
            ...
            disabled={
              !hasEmployeeProfile || isTodayError || isClockedIn || isMutating || isTodayPending
            }
