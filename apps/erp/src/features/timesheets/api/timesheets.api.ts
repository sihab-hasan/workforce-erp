...
export function createTimesheetsApi(http: ReturnType<typeof createHttpClient>) {
  return {
    ...
    getTodayStatus(employeeId?: string): Promise<ApiResponse<TodayTimesheetStatus>> {
      ...
      return http.get<ApiResponse<TodayTimesheetStatus>>("/api/v1/timesheets/today", params);
    },
    ...
    clockIn(payload?: ClockInPayload): Promise<ApiResponse<Timesheet>> {
      return http.post<ApiResponse<Timesheet>>("/api/v1/timesheets/clock-in", payload ?? {});
    },
    ...
    clockOut(payload?: ClockOutPayload): Promise<ApiResponse<Timesheet>> {
      return http.post<ApiResponse<Timesheet>>("/api/v1/timesheets/clock-out", payload ?? {});
    },
     ...
     */
    update(id: string, payload: Partial<Timesheet>): Promise<ApiResponse<Timesheet>> {
      return http.put<ApiResponse<Timesheet>>(`/api/v1/timesheets/${id}`, payload);
    },
