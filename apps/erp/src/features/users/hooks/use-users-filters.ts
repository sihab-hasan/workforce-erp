...
export function useUsersFilters() {
  ...
  const filters = useMemo<UsersFiltersState>(
    () => ({
      ...
      status: (searchParams.get("status") as UserAccountStatus | "all" | null) ?? "all",
  ...
  const onFiltersChange = useCallback(
    (partial: Partial<UsersFiltersState>) => write({ ...filters, ...partial }, 1),
    [filters, write],
  );
  ...
  return {
    ...
    pageSize: PAGE_SIZE,
    queryFilters,
    isDirty: filters.search !== "" || filters.status !== "all" || filters.role !== "all",
    onFiltersChange,
    onReset,
    onPageChange,
  };
