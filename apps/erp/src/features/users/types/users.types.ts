...
export interface EmployeeOption {
  ...
  email?: string;
  /** Existing account link, if any. Used to avoid accidental reassignment. */
...
/**
 * Full user account as returned by `GET /api/v1/users/{id}`.
 */
export interface User {
  ...
  /** Present on invite responses; false means the account exists but mail delivery failed. */
  invitation_delivered?: boolean | null;
}
...
/**
 * Payload for `POST /api/v1/users` — invites a new user.
...
export interface InviteUserPayload {
   ...
   * If omitted the link can be established later via `PUT /api/v1/users/{id}`.
   */
  employee_id?: string | null;
}

/**
 * Payload for `PUT /api/v1/users/{id}` — updates basic account information.
 * All fields are optional; only supplied fields are changed.
 */
export interface UpdateUserPayload {
  name?: string;
  /** Login email. Backend rejects unsafe cross-organization identity changes. */
  email?: string;
  role?: UserRole;
  organization_id?: string;
