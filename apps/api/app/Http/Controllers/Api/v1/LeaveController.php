...
class LeaveController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->filled('start_date')) $query->whereDate('end_date', '>=', $request->input('start_date'));
        if ($request->filled('end_date')) $query->whereDate('start_date', '<=', $request->input('end_date'));
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $isManager = in_array($role, self::MANAGER_ROLES, true);
        if (! $employeeId || (! $isManager && ! in_array($employeeId, $ownEmployeeIds, true))) abort(403, 'You cannot submit leave for this employee.');
        $employee = Employee::query()->whereKey($employeeId)->where('organization_id', $org->id)->firstOrFail();
        if ($branch && (int) $employee->branch_id !== (int) $branch->id) abort(422, 'Employee does not belong to the selected company.');
        ...
        if ($overlap) abort(409, 'This employee already has an overlapping leave request.');
    ...
    public function cancel(Request $request, LeaveRequest $leaveRequest): JsonResponse
    {
        ...
        if (! in_array($leaveRequest->status, ['pending','approved'], true)) abort(409, 'This leave request cannot be cancelled.');
    ...
    private function review(Request $request, LeaveRequest $leaveRequest, string $status): JsonResponse
    {
        ...
        if ($leaveRequest->status !== 'pending') abort(409, 'Only pending leave requests can be reviewed.');
    ...
    private function assertScoped(Request $request, LeaveRequest $leave): void
    {
        ...
        if ($branch) abort_unless((int) $leave->branch_id === (int) $branch->id, 404);
    }
    ...
    private function assertAccess(Request $request, LeaveRequest $leave, bool $ownRequired = false): void
    {
        ...
        $role = $this->scope->role($request);
        if (! $ownRequired && in_array($role, self::MANAGER_ROLES, true)) return;
