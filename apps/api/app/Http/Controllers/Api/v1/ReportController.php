...
class ReportController extends Controller
{
    ...
    private function context(Request $request): array
    {
        ...
        $this->scope->authorize($request, 'report.view');
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        $this->dataScope->applyEmployeeScope($employees, $request->user(), (int) $org->id);

        $departments = Department::query();
        $this->dataScope->applyDepartmentScope($departments, $request->user(), (int) $org->id);
        ...
        if ($branch) {
            $employees->where('branch_id', $branch->id);
            $departments->where('branch_id', $branch->id);
            $leaves->where('branch_id', $branch->id);
            $timesheets->whereHas('employee', fn ($e) => $e->where('branch_id', $branch->id));
        }
        ...
        return $this->successResponse([
            ...
            'departments' => [
                'total' => (clone $departments)->count(),
                'active' => (clone $departments)->where('is_active', true)->count(),
            ],
            'leave' => [
                'pending' => (clone $leaves)->where('status', 'pending')->count(),
                'approved_this_month' => (clone $leaves)->where('status', 'approved')->whereYear('start_date', now()->year)->whereMonth('start_date', now()->month)->count(),
            ],
            'timesheets' => [
                'this_month_hours' => (float) (clone $timesheets)->whereYear('date', now()->year)->whereMonth('date', now()->month)->sum('total_hours'),
    ...
    public function employees(Request $request): JsonResponse
    {
        ...
        $this->dataScope->applyEmployeeScope($query, $request->user(), (int) $org->id);
        ...
        $byDepartment = (clone $query)
            ...
            ->selectRaw("COALESCE(departments.name, 'Unassigned') as label, COUNT(employees.id) as total")
            ->groupBy('label')
            ->orderByDesc('total')
            ->get();
    ...
    public function departments(Request $request): JsonResponse
    {
        ...
        $rows = $query->with(['manager', 'branch'])
            ->withCount('employees')
            ->orderBy('name')
            ->get()
            ->map(fn ($dept) => [
    ...
    public function leave(Request $request): JsonResponse
    {
        ...
        $byType = (clone $query)
            ...
            ->selectRaw('leave_types.name as label, COUNT(*) as total, SUM(total_days) as days')
            ->groupBy('leave_types.name')
            ->get();

        return $this->successResponse([
            'requests' => (clone $query)->count(),
            'days' => (float) (clone $query)->where('status', 'approved')->sum('total_days'),
            'by_status' => $byStatus,
            'by_type' => $byType,
        ]);
    }
    ...
    public function timesheets(Request $request): JsonResponse
    {
        ...
        if ($branch) {
            $query->whereHas('employee', fn ($e) => $e->where('branch_id', $branch->id));
        }
        ...
        return $this->successResponse([
            ...
            'hours' => (float) (clone $query)->sum('total_hours'),
            'by_status' => $byStatus,
            'daily' => $daily,
        ]);
    }
