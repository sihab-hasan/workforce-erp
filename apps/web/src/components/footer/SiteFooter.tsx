...
import { footerNavigationGroups, siteRoutes } from "#config/site-map";
...
export default function SiteFooter() {
  ...
  useGSAP(
    () => {
      ...
      timeline
        .from("[data-footer-brand]", { autoAlpha: 0, duration: 0.82, y: 34 }, 0.16)
        .from("[data-footer-group]", { autoAlpha: 0, duration: 0.62, stagger: 0.1, x: 26 }, 0.42)
        .from("[data-footer-meta]", { autoAlpha: 0, duration: 0.62, y: 16 }, 0.6)
        .from(
          "[data-footer-node]",
          {
            duration: 0.4,
  ...
  return (
    <footer ref={footerRef} className="relative overflow-hidden bg-primary text-primary-foreground">
      ...
      <Container className="relative py-12 sm:py-16">
        ...
        >
          <p>© {new Date().getFullYear()} Workforce ERP. All rights reserved.</p>
          <Link
          ...
          >
            <span className="relative flex size-3.5 items-center justify-center">
              ...
              />
              <CircleCheck aria-hidden="true" className="relative size-3.5 text-emerald-500" />
            </span>
            System status
          </Link>
        </div>
