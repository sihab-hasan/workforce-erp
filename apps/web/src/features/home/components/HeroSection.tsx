...
const previewNavigationItems: ReadonlyArray<readonly [string, LucideIcon]> = [
  ["Home", Users],
  ["People", Users],
  ["Time", Clock3],
  ["Leave", CalendarDays],
...
export function HeroSection({ className }: HeroSectionProps) {
  ...
  useGSAP(
    () => {
      ...
      const path = heroRef.current?.querySelector<SVGPathElement>("[data-hero-path]");
    ...
    >
      <Container className="grid min-h-[calc(100svh-4.5rem)] items-center gap-14 py-16 lg:grid-cols-[minmax(0,0.82fr)_minmax(38rem,1.18fr)] lg:gap-8 lg:py-20 xl:gap-14">
        <div className="relative z-10 max-w-2xl">
          ...
          >
            Bring people, time, payroll, and performance together—so every team can move with
            confidence.
          </p>
          ...
          <p data-hero-copy className="mt-6 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <div className="grid min-h-[510px] grid-cols-[7.25rem_minmax(0,1fr)] sm:grid-cols-[8.5rem_minmax(0,1fr)]">
          <aside className="border-r border-border bg-muted/25 p-3 sm:p-4">
            ...
            <nav aria-label="Product preview" className="mt-6 space-y-1">
              {previewNavigationItems.map(([label, Icon], index) => (
                <div
                  key={label}
                  className={cn(
          ...
          <div className="relative p-4 sm:p-6">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs text-muted-foreground">Tuesday, 18 July</p>
                <p className="mt-1 font-heading text-lg font-bold">Good morning, Olivia</p>
              </div>
            ...
            <div className="rounded border border-border bg-background p-4 shadow-sm sm:p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-heading text-sm font-bold sm:text-base">Attendance overview</p>
              ...
              <div className="mt-4 grid grid-cols-4 divide-x divide-border rounded-lg border border-border">
                {[
                  ...
                  <div key={label} className="px-2 py-3 sm:px-3">
                    <p className="font-heading text-base font-bold sm:text-xl">{value}</p>
        ...
        {employees.map((employee) => (
          <div key={employee.name} className="flex items-center gap-2 py-1.5">
            ...
            <span className="min-w-0">
              <span className="block truncate text-[9px] font-semibold">{employee.name}</span>
